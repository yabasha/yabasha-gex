...
_Generated by GEX_`
    ...
    it('should install packages when --install flag is used', async () => {
      ...
      expect(mockExecFileAsync).toHaveBeenCalledWith(
        'npm',
        ['i', '--ignore-scripts', 'axios@1.6.0', 'commander@12.1.0'],
    ...
    it('should handle dev dependencies in report', async () => {
      ...
      expect(mockExecFileAsync).toHaveBeenCalledWith(
        'npm',
        ['i', '-D', '--ignore-scripts', 'vitest@2.1.9'],
        expect.any(Object),
      )
  ...
  describe('audit command', () => {
    it('prints the audit table to stdout when no --out-file is provided', async () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('1 high'))
      expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('lodash'))
      expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('"audit_summary"'))
      expect(mockWriteFile).not.toHaveBeenCalled()
    })

    it('writes the report to file when --out-file is provided and exit-code is 0 below threshold', async () => {
      process.argv = ['node', 'cli.js', 'audit', '-o', 'audit.json', '--fail-on', 'critical']
      mockMkdir.mockResolvedValue(undefined)
      mockWriteFile.mockResolvedValue()
      mockExecFileAsync.mockImplementation(async (cmd: string, args: string[]) => {
        if (cmd === 'npm' && args[0] === 'audit') {
      ...
      expect(mockWriteFile).toHaveBeenCalledWith(
        'audit.json',
        expect.stringContaining('"audit_summary"'),
        'utf8',
      )
      expect(process.exitCode).toBe(0)
    })

    it('exits non-zero when --fail-on threshold is met', async () => {
      process.argv = ['node', 'cli.js', 'audit', '-o', 'audit.json', '--fail-on', 'high']
      mockMkdir.mockResolvedValue(undefined)
      mockWriteFile.mockResolvedValue()
      mockExecFileAsync.mockImplementation(async (cmd: string, args: string[]) => {
        if (cmd === 'npm' && args[0] === 'audit') {
    ...
    it('soft-fails when audit throws and exits 0 without --fail-on', async () => {
      process.argv = ['node', 'cli.js', 'audit', '-o', 'audit.json']
      mockMkdir.mockResolvedValue(undefined)
      mockWriteFile.mockResolvedValue()
      mockExecFileAsync.mockImplementation(async (cmd: string, args: string[]) => {
        if (cmd === 'npm' && args[0] === 'audit') {
          ...
          error.stdout = ''
          error.stderr = 'getaddrinfo ENOTFOUND registry.npmjs.org'
          throw error
        }
        return { stdout: '', stderr: '' }
      })
    ...
    it('exits with code 2 when --fail-on threshold itself is invalid', async () => {
      process.argv = ['node', 'cli.js', 'audit', '-o', 'audit.json', '--fail-on', 'extreme']
      mockMkdir.mockResolvedValue(undefined)
      mockWriteFile.mockResolvedValue()
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        expect.stringContaining('Invalid --fail-on value: extreme'),
      )
      expect(process.exitCode).toBe(2)
      expect(mockWriteFile).not.toHaveBeenCalled()
    })

    it('exits non-zero when audit fails and --fail-on is set', async () => {
      process.argv = ['node', 'cli.js', 'audit', '-o', 'audit.json', '--fail-on', 'low']
      mockMkdir.mockResolvedValue(undefined)
      mockWriteFile.mockResolvedValue()
      mockExecFileAsync.mockImplementation(async (cmd: string, args: string[]) => {
        if (cmd === 'npm' && args[0] === 'audit') {
